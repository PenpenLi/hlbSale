            ...
            else
            {
                ...
                do {
                    ...
                    if (pMtd->usePack)
                    {
                        if (pMtd->selectPack2)
                        {
                            msg_data = MsgPack::pack2(pMtd->_postData);
                        }
                        else
                        {
                            msg_data = MsgPack::pack(pMtd->_postData);
                        }
...
httpNet::httpNet():_failedThing(true)
{
}
...
void httpNet::_process(msgThreadData * pMtd)
{
    if (pMtd->_isDiscard == false)
    {
        ...
        if (pMtd->_succeed && pMtd->usePack)
        {
            ...
            {
                int sss = 0;
                dataString = MsgPack::unpack2_new(dataString, &sss);
            }
            else
            {
                dataString = MsgPack::unpack(dataString);
            }
        }
        pStack->pushString(dataString.c_str(), dataString.size());
