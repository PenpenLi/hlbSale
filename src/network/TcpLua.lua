...
local TcpLua = {}
setmetatable(TcpLua, {__index = _G})
setfenv(1, TcpLua)
...
--TcpLuaNotifyEnum of "NetCommon.h"
TcpLua.NotifyConnectingLoginServerFail = 200002
TcpLua.NotifyConnectedLoginServer      = 200003
TcpLua.NotifyConnectingGameServerFail  = 200004
TcpLua.NotifyConnectedGameServer       = 200005
TcpLua.NotifyDisconnected              = 200006
TcpLua.NotifyRecvMsg                   = 200007
...
--default handlers
registNotifyHandler(TcpLua.NotifyConnectingLoginServerFail, nil, dumpCallback)
registNotifyHandler(TcpLua.NotifyConnectedLoginServer, nil, dumpCallback)
registNotifyHandler(TcpLua.NotifyConnectingGameServerFail, nil, dumpCallback)
registNotifyHandler(TcpLua.NotifyConnectedGameServer, nil, dumpCallback)
registNotifyHandler(TcpLua.NotifyDisconnected, nil, dumpCallback)
registNotifyHandler(TcpLua.NotifyRecvMsg, nil, onRecvMsgData)
  ...
  if m_isConnected == true then 
    ...
    if duration >= m_maxHeartBeatTimeout then -- timeout      
      ...
      if m_heartBeatTimerId then 
        cc.Director:getInstance():getScheduler():unscheduleScriptEntry(m_heartBeatTimerId) 
        m_heartBeatTimerId = nil 
      end          
      sendReq(ReqDisconnect) --TcpLuaManager will change to disconnecting state, then game server task break, and notify user
    end
...
function sendHeartBeat()
  -- print("sendHeartBeat")
  sendMessage(g_consts.NetMsg.HeartBeatReq, {id = g_playerData.getData().id})
end
---------------------------------------------------------------
...
function startHeartBeat()
  ...
  registMsgCallback(g_consts.NetMsg.HeartBeatRsp, TcpLua, onRecvHeartBeatRsp) 
end 
